...
inline void real_main(int argc, char **argv)
{
    ...
    mongocxx::client conn{mongocxx::uri{"mongodb://brian:mocha11@192.168.86.59/snowdb?authSource=admin"}};
    
    mongocxx::collection sampleCollection = conn["snowdb"]["OneCell2"];
    //setCollection is smetIO method
    SmetIO::setCollection(sampleCollection);
    //End Mongo
	...
	try { //main integration loop
        cout << "AlpineMain starting setStaticData()" << endl;
		setStaticData(cfg, io, dem, landuse, vec_pts);
        cout << "AlpineMain starting setModules()" << endl;
		setModules(cfg, io, dem, landuse, vec_pts, drift, eb, snowpack, da, runoff);
        cout << "AlpineMain starting AlpineControl control()" << endl;
		AlpineControl control(snowpack, drift, eb, da, runoff, cfg, dem);
        cout << "AlpineMain starting control.setNoCompute()" << endl;
		control.setNoCompute(nocompute);
        cout << "AlpineMain starting control.Run()" << endl;
    ...
    cout << "AlpineMain finished try catch" << endl;
	cleanDestroyAll(drift, eb, snowpack, da, runoff);
