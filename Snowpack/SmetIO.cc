...
void SmetIO::writeTimeSeries(const SnowStation& Xdata, const SurfaceFluxes& Sdata, const CurrentMeteo& Mdata, const ProcessDat& Hdata, const double wind_trans24)
{
    ...
    timestamp = originalTime.substr(5, 8);
    ...
    printf("I: %d, J: %d, cell_lat: %s, cell_lon: %s", timestamp.c_str(), I, J, Cell_Lat, Cell_Lon);
...
void SmetIO::getSpecialValues(size_t& currentSoilNode, vector<double> density, vector<double> viscosity, vector<double> thickness, vector<double> hardness, double& SkiD, double& BootD, double& SkinD, double& firmest) {
  ...
  for (int i = density.size() - 1; i > -1; i--) {
      currentResistance = log(density[i] * viscosity[i] * thickness[i]);
