...
SmetIO::SmetIO(const SnowpackConfig& cfg, const RunInfo& run_info)
        : fixedPositions(), outpath(), o_snowpath(), experiment(), inpath(), i_snowpath(), sw_mode(),
          ...
          out_load(false), out_stab(false), out_canopy(false), out_soileb(false), Cell_Lat("40.1"), Cell_Lon("-106.1")
{
...
SmetIO& SmetIO::operator=(const SmetIO& source) {
	if (this != &source) {
		...
		Cell_Lat = source.Cell_Lat;
		Cell_Lon = source.Cell_Lon;
	}
	return *this;
}
...
{
    cout << "SmetIO starting writeTimeSeries()" << endl;
    ...
    cout << "SmetIO starting set values for find and update" << endl;
    // Values
    std::string timestamp = Mdata.date.toString(Date::ISO_Z);
    std::string originalTime = timestamp;
	timestamp = originalTime.substr(5, 9);
    timestamp.pop_back();
    printf("Original timestamp to find: %s\nFixed timestamp: %s\n", originalTime.c_str(), timestamp.c_str());
    int I = Xdata.meta.position.getGridI();
    int J = Xdata.meta.position.getGridJ();
    ...
    vector<double> temp;
    vector<double> layerLiquidWater;
    vector<bool> dendricity;
    vector<short unsigned int> grainType;

	size_t 	currentSoilNode = Xdata.SoilNode; // Top soil node 

    const vector<ElementData>& EMS = Xdata.Edata;
	for (size_t e = 0; e < Xdata.getNumberOfElements(); e++) {
		thick.push_back(EMS[e].L);
		layerLiquidWater.push_back(EMS[e].theta[WATER]);
		dendricity.push_back(EMS[e].dd);
		visc.push_back(EMS[e].k[SETTLEMENT]);
		density.push_back(EMS[e].Rho);
		temp.push_back(EMS[e].Te);
		hardness.push_back(EMS[e].hard);
		grainType.push_back(EMS[e].type);
		quality.push_back(log(1 + EMS[e].Rho * EMS[e].k[SETTLEMENT]));
	}
    // End values
    cout << "SmetIO finished setting values for find and update" << endl;
    
    int rec_index = 0;
    double SkiD; double BootD; double SkinD; double firmest; double surfaceLayerHardness;
    if (hardness.size() > 0) {
        surfaceLayerHardness = hardness[0];
    } else {
        surfaceLayerHardness = -1;
    }
    
    
    cout << "SmetIO starting getSpecialValues()" << endl;
    getSpecialValues(currentSoilNode, density, visc, thick, hardness, SkiD, BootD, SkinD, firmest);
    printf("SmetIO found special values:\nSkiD: %g, BootD: %g, SkindD: %g\nSmetIO starting updateCollection()\n", SkiD, BootD, SkinD);
    cout << "Starting update collection\nTimestamp: " << timestamp << endl;
    printf("I: %d, J: %d, cell_lat: %s, cell_lon: %s", I, J, Cell_Lat, Cell_Lon);
    updateCollection(timestamp, I, J, Cell_Lat, Cell_Lon,
                     /*TA*/     airTemp,
                     /*VW*/     windSpeed,
                     /*DW*/     windDirection,
                     /*VW_MAX*/ gustSpeed,
    ...
    cout << "SmetIO finished updateCollection()" << endl;
    ...
    quality.clear();
    temp.clear();
    layerLiquidWater.clear();
    dendricity.clear();
    grainType.clear();

    cout << "SmetIO starting writeTimeSeriesData()" << endl;
...
void SmetIO::setCollection(mongocxx::collection newCollection) {
    collection = newCollection;
}

void SmetIO::updateCollection(
   ...
   cout << "I: " << I << " J: " << J << " cell_lat: " << Cell_Lat<< " cell_lon: "<< Cell_Lon << endl;
    ...
    collection.update_one(document{}
                          ...
                          << recordToUpdate + ".Qual" << QualArr.view()
                          << recordToUpdate + ".Th" << ThArr.view()
                          << recordToUpdate + ".Hd" << HdArr.view()
                          << recordToUpdate + ".Den" << DenArr.view()
                          << recordToUpdate + ".Te" << TeArr.view()
                          << recordToUpdate + ".LW" << LWArr.view()
                          << recordToUpdate + ".DD" << DDArr.view()
                          << recordToUpdate + ".Visc" << ViscArr.view()
                          << recordToUpdate + ".Type" << TypeArr.view()
...
void SmetIO::getSpecialValues(size_t& currentSoilNode, vector<double> density, vector<double> viscosity, vector<double> thickness, vector<double> hardness, double& SkiD, double& BootD, double& SkinD, double& firmest) {

    if (hardness.size() > 0) {
        firmest = hardness[hardness.size() - 1];
    } else {
        cout << "no values in hardness vector" << endl;
    }
    
    SkiD = 0.0;
    BootD = 0.0;
    SkinD = 0.0;
    double skiPen = 60.0 * 0.15/(1.76 * 0.130);
    double bootPen = 60.0 * .15/(0.290 * 0.130);
    double skinPen = 60.0 * 0.03/ (1.76 * 0.130);
    double currentResistance = 0.0;
    
    //Need to check if there are any snow layers
    if (density.size() > currentSoilNode) {
        cout << currentSoilNode << "Snow!!!\n";
        for (int i = 0; i < currentSoilNode; i++) {
            density.erase(0);
            viscosity.erase(0);
            thickness.erase(0);
            hardness.erase(0);
    ...
    cout << "inside getSpecialValues before for loop" << endl;
	//cout <<"density size: "<<density.size()<< endl;
    printf("inside getSpecialValues before for loop\nvector sizes\n density: %u, visc: %u, thickness: %u, hardness: %u", density.size(), viscosity.size(), thickness.size(), hardness.size());


    for (int i = density.size() - 1; i > -1; i--) {
        currentResistance = log(density[i] * viscosity[i] * thickness[i]+1);
        cout << "current resitance " << currentResistance << endl;
        if (skiPen > currentResistance) {
            SkiD += thickness[i];
            skiPen -= currentResistance;
            if (hardness[i] > firmest) {
                firmest = hardness[i];
            }
        } else if (viscosity[i] > 1000.0) {
            skiPen += thickness[i] * skiPen / currentResistance;
        }
        
        if (bootPen > currentResistance) {
            BootD += thickness[i];
            bootPen -= currentResistance;
        } else if (viscosity[i] > 1000.0) {
            bootPen += thickness[i] * bootPen / currentResistance;
        }
        
        if (skinPen > currentResistance) {
            SkinD += thickness[i];
            skinPen -= currentResistance;
        } else if (viscosity[i] > 1000.0) {
            skinPen += thickness[i] * skinPen / currentResistance;
        }
    }
    cout << "inside getSpecialValues after for loop" << endl;
}
